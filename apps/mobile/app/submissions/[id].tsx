...
import { typography, textA11yProps } from "../../src/theme";
...
function toCsv(r: Row) {
  ...
  const cells = [
    ...
    [
      "SUBMITTED BY",
      r.submitter_display_name || r.submitter_email || r.created_by || "",
    ],
...
async function shareCsvNative(filename: string, text: string) {
  ...
  try {
    if (await Sharing.isAvailableAsync()) {
      ...
      Alert.alert(
        "Sharing unavailable",
        "Unable to open the iOS share sheet on this device.",
      );
    }
  } finally {
...
export default function Submission() {
  ...
  useEffect(() => {
    if (!id) return;
    (async () => {
      const rpc = await supabase.rpc("get_submission_with_submitter", {
        sub_id: id as any,
      });
      const r = Array.isArray(rpc.data)
        ? (rpc.data[0] as Row)
        : (rpc.data as Row | null);
      ...
      setPhoto1Url(
        dataRow.photo1_url ?? (await resolveSigned(dataRow.photo1_path)),
      );
      setPhoto2Url(
        dataRow.photo2_url ?? (await resolveSigned(dataRow.photo2_path)),
      );
    })();
  }, [id]);
  ...
  const downloadExcelWithPhotos = async () => {
    if (!isWeb) {
      Alert.alert(
        "Web only",
        "Excel downloads are only available from the web dashboard for now.",
      );
      return;
    }
    ...
    const { downloadSubmissionExcel } =
      await import("../../src/lib/exportExcel");
  ...
  const shareCsv = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Share failed",
        error?.message ?? "Unable to create the CSV file.",
      );
    }
  };
  ...
  const photoUrls = [
    row.photo1_url || photo1Url,
    row.photo2_url || photo2Url,
  ...
  const savePdf = async () => {
    try {
      ...
      const fn =
        (mod as any)?.downloadSubmissionPdf ||
        (mod as any)?.default?.downloadSubmissionPdf;
  ...
  return (
    ...
    >
      ...
      <Pressable
        onPress={() =>
          typeof history !== "undefined" ? history.back() : router.back()
        }
      ...
      >
        <Text
          {...textA11yProps}
          style={{ fontSize: 18, color: "#2563eb", fontWeight: "600" }}
        >
          ← Back
        </Text>
      </Pressable>
